...
urldecode() {
    local data="$1" out="" i c h
    for (( i=0; i<${#data}; i++ )); do
        c="${data:$i:1}"
        if [[ "$c" == "%" && $(( i+2 )) -lt ${#data} && "${data:$i+1:2}" =~ ^[0-9a-fA-F]{2}$ ]]; then
            printf -v h "\\x${data:$i+1:2}"
            out+="$h"
            i=$(( i+2 ))
        else
            out+="$c"
...
# Флаг страны каждой EU-ноды (из ремарки входной ссылки вида «🇪🇺 VLESS»)
declare -a NODE_FLAG
...
for (( i=0; i<COUNT; i++ )); do
    ...
    # Флаг страны ноды — первое слово ремарки («🇩🇪 VLESS» -> «🇩🇪»); иначе дефолт 🇪🇺
    NODE_FLAG[$i]="${NODE_NAME[$i]%% *}"
    if [[ -z "${NODE_FLAG[$i]}" || "${NODE_FLAG[$i]}" == "${NODE_NAME[$i]}" ]]; then
        NODE_FLAG[$i]="$(cc_to_flag EU)"
    fi
...
if [[ ! "$COUNTRY_CODE" =~ ^[A-Z]{2}$ ]]; then
    ...
    read -p "Введите код страны из 2 букв (например RU; по умолчанию RU): " cc_input
    COUNTRY_CODE=${cc_input:-RU}
...
for (( i=0; i<COUNT; i++ )); do
    # Тег номера ноды в бейдже (только когда нод несколько)
    if [ "$COUNT" -gt 1 ]; then NODE_TAG=" • нода $((i+1))"; else NODE_TAG=""; fi
...
EOF
    ...
    # Имена: флаг VPS + протокол (тип транспорта виден в клиенте)
    CLIENT_CONFIGS+="$(print_config "$OUT_REALITY_XHTTP" "$VLESS_NAME")"
    CLIENT_CONFIGS+=","
    CLIENT_CONFIGS+="$(print_config "$OUT_REALITY_VISION" "$VLESS_NAME")"
    CLIENT_CONFIGS+=","
    CLIENT_CONFIGS+="$(print_config "$OUT_DIRECT_EU" "${NODE_FLAG[$i]} VLESS")"
    ...
    link_raw="vless://${BRIDGE_UUID[$i]}@$DOMAIN:$SERVER_PORT?security=reality&type=tcp&headerType=&path=&host=&flow=xtls-rprx-vision&sni=$DOMAIN&fp=$fpBro&pbk=${xray_publicKey_vrv}&sid=${xray_shortIds_vrv}&spx=%2F#$VLESS_NAME_ENC"

    # Формат элемента: Имя (флаг+протокол)|Бейдж маршрута на странице|Ссылка
    CONFIGS_ARRAY+=( "$VLESS_NAME|мост • xhttp$NODE_TAG|$link_xhttp" )
    CONFIGS_ARRAY+=( "$VLESS_NAME|мост • raw$NODE_TAG|$link_raw" )
    CONFIGS_ARRAY+=( "${NODE_FLAG[$i]} VLESS|напрямую$NODE_TAG|${VLESS_URLS[$i]}" )
done
...
<div class="section">
    <div class="section-title">Конфиги ($COUNT × 3)</div>
EOF
