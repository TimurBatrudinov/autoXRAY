...
Bash-скрипты для развёртывания личного VPN на базе [Xray-core](https://github.com/XTLS/Xray-core). Целевая платформа — чистый Ubuntu/Debian с root. Репозиторий публичный: скрипты при установке подтягивают вспомогательные файлы с ветки `main` этого же GitHub-репозитория — **после правок локальных копий `test/*.sh` изменения не действуют, пока не запушены**.
