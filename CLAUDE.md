...
- **Certbot e-mail**: спрашивается интерактивно (валидация формата); пустой ввод → `--register-unsafely-without-email`. Аргументы собираются в массив `CERTBOT_EMAIL_ARGS`. Домен валидируется regex-ом сразу после `$1` (ловит `bash`, `--`, кириллицу).
